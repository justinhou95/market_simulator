...
######################################################################################################
class Time(tf.keras.layers.Layer):
...
class GrowingSignature(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = Sig_method.kernel_for_signature(self.num_components, self.num_features, self.num_levels, self.num_functionals)
    ...
    def call(self, X, mask=None):
        ...
        sig = Sig_method.low_rank_seq2tens(seq, self.kernel, self.num_levels, embedding_order=1, recursive_weights=False,
...
######################################################################################################    
class LS2T(tf.keras.layers.Layer):
    ...
    def call(self, X, mask=None):
        return Sig_method.low_rank_seq2tens(X, self.kernel, self.num_levels, embedding_order=self.embedding_order, recursive_weights=self.recursive_weights,
...
class GaussianDecoder(Decoder):
    ...
    def __call__(self, x):
        mean = self.net(x)
        var = tf.ones(tf.shape(mean), dtype=tf.float32)   ### variance 1 ?
