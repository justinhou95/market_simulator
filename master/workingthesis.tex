...
\begin{enumerate}[start=1,label={( Step \arabic*):}]
  \item \textbf{Time series splitting:} Split single realisation of path $(x_{0:N})$ to multiple subpaths $(x_{i:i+n})_{i=1}^{m}$. This subdivision helps to generate multiple samples for training. 
  \item \textbf{Path to signature features:} Applying the signature feature map to $m$ subpaths $(x_{i:i+n})_{i=1}$ gives $m$ samples on $\R^{d}$, where $d$ depends on the choice of siganture feature map.   
  \item \textbf{Variational autoencoder on signature features: } Train a variational autoencoder on the signature feature space and generate samples on the siganture feature space.  
...
\begin{enumerate}[(i)]
  \item Siganture type features stores path information in an efficient way by Example \ref{lcde}.
  \item Expected normalized signature characterise the law of stochastic process by Corollary \ref{equalequal}.
  ...
  \item log-sigature has the same information as sigature but store them in a more compact way, by Proposition \ref{logsignatureorder}. 
  ...
  \item the numerical log-signature is more robust than siganture, by Example  
...
\end{enumerate} 
We additional apply a lead-lag transformation \cite{flint2016discretely} to path before computing the log-signature features of the subpath. This is because the signature of a path after lead-lag transformation helps to capture the volatility of a path, which is of vital imporatance in finance. 
...
\begin{equation*}
  ...
  \end{bmatrix} \neq 0
\end{equation*} 
Unfortunately, all python packages I know available so far (including esig, iisignature, and Signatory) only support truncated discrete signature. We blenchmark with Euler approximation on a very fine grid.
\begin{figure}[H]
    \centering
...
\begin{equation}\label{eq62}
    ...
    \end{split}
\end{equation}
Where $M^{i_{1},\cdots,i_{m}}_{i,j}$ are matrixs depending on $s$. If we let $\mathcal{X} = \mathbf{Sig}_{[s,t]} \otimes X_{s}$ and $\mathcal{Y} = X_{t}$
Therefore, we can learn the relation in \eqref{eq62} by a linear regression on $(\mathcal{X},\mathcal{Y})$. Now, we consider $m = d = 2$ and 
\begin{equation*}
  V^{1}(X) = \begin{bmatrix}
    2|X^{(2)}|^{0.7}\\
    X^{(2)}
...
\end{figure}
We choose first $t=1,\cdots 1000$ to be training samples, and $t=1001,\cdots 2000$ to be test samples. We use ridge regression to learn matrixes $M^{i_{1},\cdots,i_{m}}_{i,j}$ on training samples, and generate path on testing samples with learnt matrixs. We compared the generated path with true path under the same random seed. 
\begin{figure}[H]
    \centering
